...
export function MemberCards({ members, tasks, onEditMember }: MemberCardsProps) {
    ...
    const checkIsAgent = (m: Member) => m.role === 'AGENT' || m.email.endsWith('@odin.agent');
    
    const sorted = [...members].sort((a, b) =>
        (taskCountById.get(b.id) ?? 0) - (taskCountById.get(a.id) ?? 0)
    );

    const humans = sorted.filter(m => !checkIsAgent(m));
    const agents = sorted.filter(m => checkIsAgent(m));
    ...
    const renderCard = (member: Member) => {
        ...
        const avgWorkTime = memberTasks.length > 0
            ...
            : 0;
        const isAgent = checkIsAgent(member);

        return (
            <Card key={member.id} className="border-border">
                <CardContent className="p-5">
                    <div className="flex items-center gap-3 mb-4">
                        <UserAvatar member={member} size="lg" className="size-10" />
                        <div className="flex-1 min-w-0">
                            <div className="text-sm font-semibold flex items-center gap-1.5">
                                {member.fullName}
                                {isAgent && (
                                    <Badge variant="secondary" className="text-[10px] px-1.5 py-0 bg-indigo-100 text-indigo-700 dark:bg-indigo-950 dark:text-indigo-400">Agent</Badge>
                                )}
                            </div>
    ...
    return (
        <div className="space-y-6 mb-8">
            {humans.length > 0 && (
                <div>
                    <h3 className="text-xs font-semibold text-muted-foreground mb-3 uppercase tracking-wider px-1">Team Members</h3>
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-4">
                        {humans.map(renderCard)}
                    </div>
                </div>
            )}
            {agents.length > 0 && (
                <div>
                    <h3 className="text-xs font-semibold text-muted-foreground mb-3 uppercase tracking-wider px-1">AI Agents</h3>
                    <div className="grid grid-cols-[repeat(auto-fill,minmax(320px,1fr))] gap-4">
